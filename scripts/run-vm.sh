...
Environment (forwarded if set):
  DISTRO, VM_MEMORY, VM_CPUS, VM_DISK_SIZE, VM_DISPLAY, VM_VNC_PORT, VM_NOVNC_PORT, VM_BASE_IMAGE,
  VM_BLANK_DISK, VM_BOOT_ISO, VM_BOOT_ORDER, VM_CLOUD_INIT, VM_ARCH, VM_CPU_MODEL,
...
for v in DISTRO VM_MEMORY VM_CPUS VM_DISK_SIZE VM_DISPLAY VM_VNC_PORT VM_NOVNC_PORT VM_BASE_IMAGE VM_BLANK_DISK VM_BOOT_ISO VM_BOOT_ORDER VM_CLOUD_INIT VM_ARCH VM_CPU_MODEL VM_PASSWORD VM_SSH_PUBKEY EXTRA_ARGS; do
  forward_env "$v"
done
